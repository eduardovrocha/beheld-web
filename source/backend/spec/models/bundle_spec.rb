...
RSpec.describe Bundle, type: :model do
  ...
  describe ".schema_version" do
    it "retorna :v2 quando l1 e l2 presentes" do
      expect(Bundle.schema_version(v2_inner_payload)).to eq(:v2)
    end

    it "retorna :v1 quando signals presente" do
      expect(Bundle.schema_version(v1_inner_payload)).to eq(:v1)
    end

    it "retorna :unknown quando nenhum dos dois" do
      expect(Bundle.schema_version({ "scores" => base_scores })).to eq(:unknown)
    end

    it "retorna :unknown para entrada não-hash" do
      expect(Bundle.schema_version(nil)).to eq(:unknown)
      expect(Bundle.schema_version("not a hash")).to eq(:unknown)
      expect(Bundle.schema_version([])).to eq(:unknown)
    end

    it "prefere :v2 quando ambos l1+l2 e signals estão presentes (futuro misto)" do
      mixed = v2_inner_payload.merge("signals" => {})
      expect(Bundle.schema_version(mixed)).to eq(:v2)
    end
  end
  ...
  describe ".valid_payload?" do
    it "aceita payload v1 com signals" do
      expect(Bundle.valid_payload?(v1_inner_payload)).to be(true)
    end

    it "aceita payload v2 com l1 e l2" do
      expect(Bundle.valid_payload?(v2_inner_payload)).to be(true)
    end
